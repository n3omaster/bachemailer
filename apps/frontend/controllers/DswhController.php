...
class DswhController extends Controller
{
    ...
    public function actionPostal()
    {
        ...
        if (in_array($event['event'], array('MessageDeliveryFailed', 'MessageDelayed', 'MessageHeld'))) {
        	...
        	$possibleMessageIds = array_filter(array(
		        isset($event['payload']['message']['token']) ? $event['payload']['message']['token'] : '',
		        isset($event['payload']['message']['message_id']) ? $event['payload']['message']['message_id'] : ''
	        ));
        ...
        if ($event['event'] == 'MessageBounced') {
	        ...
	        $possibleMessageIds = array_filter(array(
		        isset($event['payload']['original_message']['token']) ? $event['payload']['original_message']['token'] : '',
		        isset($event['payload']['original_message']['message_id']) ? $event['payload']['original_message']['message_id'] : ''
	        ));
	...
	{
		$postedEvents = file_get_contents("php://input");
		if (empty($postedEvents)) {
			Yii::app()->end();
		}

		$events = array_filter(array_merge(
			(array)CJSON::decode($postedEvents),
			(array)array_filter(array_map(array('CJSON', 'decode'), array_filter((array)explode(PHP_EOL, $postedEvents))))
		), function($event) {
		...
		foreach ($events as $event) {
			...
			$event[$this->pmtaMessageIdHeaderKey] = str_replace(array('<', '>'), '', $event[$this->pmtaMessageIdHeaderKey]);
			...
			if (in_array($event['type'], array('f'))) {
				...
				if (empty($count)) {
					...
					$complaintLog->campaign_id   = $campaign->campaign_id;
					$complaintLog->subscriber_id = $subscriber->subscriber_id;
					$complaintLog->message       = 'Abuse complaint via PMTA!';
					$complaintLog->save(false);
				}
    ...
    public function processSendinblue()
    {
        ...
        if (in_array($event['event'], array('spam', 'unsubscribed'))) {
