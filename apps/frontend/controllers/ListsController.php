...
class ListsController extends Controller
{
    ...
    public function actionSubscribe($list_uid, $subscriber_uid = null)
    {
        ...
        $searchReplace = array(
	        ...
	        '[SUBMIT_BUTTON]'       => CHtml::button(Yii::t('lists', 'Subscribe'), array('type' => 'submit', 'class' => 'btn btn-primary btn-flat')),
        );
	    ...
	    if ($request->isPostRequest && strlen($request->getPost($honeypotFieldName, '')) > 0) {
		    ...
		    return $this->render('display_content', compact('content'));
	    }
	    //
        ...
        if ($request->isPostRequest) {
        	...
        	if (!$mutex->acquire($mutexKey, 20)) {
        		throw new CHttpException(Yii::t('lists', 'Please try to resubmit the form once again!'));
	        }
    ...
    public function actionUnsubscribe($list_uid, $subscriber_uid = null, $campaign_uid = null, $type = null)
    {
        ...
        $searchReplace = array(
	        ...
	        '[LIST_INTERNAL_NAME]'  => $list->name,
	        '[LIST_UID]'            => $list->list_uid,
	        '[SUBMIT_BUTTON]'       => CHtml::button(Yii::t('lists', 'Unsubscribe'), array('type' => 'submit', 'class' => 'btn btn-default')),
        );
    ...
    public function actionUnsubscribe_confirm($list_uid, $subscriber_uid, $campaign_uid = null)
    {
	    ...
	    $searchReplace = array(
	        ...
	        '[LIST_INTERNAL_NAME]'  => $list->name,
	        '[LIST_UID]'            => $list->list_uid,
	        '[SUBSCRIBE_URL]'       => Yii::app()->apps->getAppUrl('frontend', sprintf('lists/%s/subscribe/%s', $list->list_uid, $subscriber->subscriber_uid), true),
        );
