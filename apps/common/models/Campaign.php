...
class Campaign extends ActiveRecord
{
    ...
    public function search()
    {
        ...
        return new CActiveDataProvider(get_class($this), Yii::app()->hooks->applyFilters('campaign_model_search_data_provider_config', array(
            'criteria'      => $criteria,
            'pagination'    => array(
    ...
    public function copy($doTransaction = true)
    {
        ...
        try {
            ...
            $option->cronjob_rescheduled        = CampaignOption::TEXT_NO;
    ...
    public function tryReschedule()
    {
	    ...
	    CampaignOption::model()->updateByPk((int)$campaign->campaign_id, array(
        ...
        try {
            ...
            $currentTime       = new $dateTimeClass($this->send_at, new $dateTimeZoneClass(Yii::app()->timeZone));
            ...
            if (!empty($this->customer->timezone)) {
                ...
                $customerTime->setTimezone(new $dateTimeZoneClass($this->customer->timezone));

                $cron = call_user_func(array('\Cron\CronExpression', 'factory'), $this->option->cronjob);
                $campaign->send_at = $cron->getNextRunDate($customerTime)->format('Y-m-d H:i:s');

                $currentTime = new $dateTimeClass($campaign->send_at, new $dateTimeZoneClass($this->customer->timezone));
                $currentTime->setTimezone(new $dateTimeZoneClass(Yii::app()->timeZone));
            ...
            $ok = Yii::app()
                     ->getDb()
                     ->createCommand()
                     ->update($this->tableName(), $attributes, 'campaign_id = :cid', array(':cid' => (int)$campaign->campaign_id));
        ...
        if ($ok) {
	        ...
	        try {
		        Yii::app()->getDb()
		           ->createCommand()
		           ->update($this->option->tableName(), $attributes, 'campaign_id = :cid', array(':cid' => (int)$this->campaign_id));
	        } catch (Exception $e) {
		        Yii::log($e->getMessage(), CLogger::LEVEL_ERROR);
	        }
        }
    ...
    public function getBulkActionsList()
    {
        $actions = array(
            ...
            self::BULK_ACTION_COMPARE_CAMPAIGNS   => Yii::t('app', 'Compare campaigns'),
        );
        
        if (!empty($this->customer_id)) {
        	$actions[self::BULK_ACTION_SEND_TEST_EMAIL]     = Yii::t('app', 'Send test email');
        	$actions[self::BULK_ACTION_SHARE_CAMPAIGN_CODE] = Yii::t('app', 'Share with code');
        }
        ...
        // since 1.9.17
        $actions = Yii::app()->hooks->applyFilters('campaign_model_get_bulk_actions_list', $actions, $this);
