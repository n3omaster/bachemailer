...
class ListImportCommand extends ConsoleCommand
{
			...
			if (in_array($extension, array('csv', 'txt')) && is_file($file)) {
				...
				if ($canSendEmail && ($server = DeliveryServer::pickServer())) {
                    ...
                    if (empty($this->errorMessages)) {
                        $errorsSummary = Yii::t('list_import', 'List import finished without errors.');
                    } else {
                        $errors = array_slice($this->errorMessages, 0, 10);
                        $errors = CMap::mergeArray($errors, array('[...]'));
                        $errors = CMap::mergeArray($errors, array_slice($this->errorMessages, -10, 10));
                        $errorsSummary = Yii::t('list_import', 'List import finished with errors, here is a small excerpt: {errors}', [
	...
	protected function processText(array $params)
	{
		...
		$rounds = ceil($totalFileRecords / $importAtOnce);
		for ($rCount = 1; $rCount <= $rounds; $rCount++) {
	...
	protected function renderMessage($data = array())
	{
        ...
        if (
            (isset($data['type']) && in_array($data['type'], array('error'))) ||
            (isset($data['result']) && in_array($data['result'], array('error')))
        ) {
	        $out = '';
	        if (isset($data['type'])) {
